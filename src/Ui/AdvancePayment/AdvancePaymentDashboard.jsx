  ...
  // All data now comes from the backend via advancePaymentService
  // ─────────────────────────────────────────────────────────────────────────────
  ...
  export default function AdvancePaymentDashboard() {
    ...
    useEffect(() => {
      const t = setTimeout(fetchRequests, search ? 400 : 0); // debounce search
      return () => clearTimeout(t);
    }, [fetchRequests]);

    // ── Actions ────────────────────────────────────────────────────────────────
  // In AdvancePaymentDashboard.jsx — replace the approve function
  ...
  const approve = async (id) => {
    try {
      ...
      setViewedReq(null);
      setActiveTab("approved"); // ← ADD THIS LINE
    } catch (err) {
    ...
    const reject = async (id) => {
      try {
        await advancePaymentService.rejectRequest(id);
        await Promise.all([fetchStats(), fetchRequests()]);
        setViewedReq(null);
      } catch (err) {
