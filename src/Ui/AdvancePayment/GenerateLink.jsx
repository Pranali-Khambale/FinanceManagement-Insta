...
// Changes:
//   • Removed org_to_emp payment type (only emp_to_emp + other remain)
//   • Link validity = 30 days (1 month), reusable — multiple screenshots allowed
//   • Email auto-sent on generate (no manual send button)
// ─────────────────────────────────────────────────────────────────────────────
...
export default function GenerateLinkModal({ onClose }) {
  ...
  const handleGenerate = async () => {
    // Email is required since we auto-send
    if (!email) {
      setEmailErr("Employee email is required to send the link");
      return;
    }
    ...
    try {
      const res = await advancePaymentService.createLink({
        ...
        expires_in_days: 30, // 1-month validity
        multi_use: true, // reusable — multiple screenshot submissions
      });
      ...
      // Auto-send email immediately after generation
      try {
        await advancePaymentService.sendLinkEmail({
          token,
          email,
          payment_type_key: retPtKey,
        });
      } catch (mailErr) {
        console.warn("Email send failed:", mailErr.message);
        // Still proceed to step 2 — link is generated
      }
    ...
    <>
      ...
      >
        <div style={S.modal}>
          ...
          <div style={S.header}>
            ...
            >
              <div>
                ...
                >
                  Employee fills the form &amp; uploads screenshots · valid 1
                  month
                </p>
              </div>
          ...
          {step === 1 && (
            <div style={S.body}>
              {/* Payment type grid — only emp_to_emp + other */}
              <div>
              ...
              >
                ...
                >
                  Valid for{" "}
                  <strong style={{ color: "#334155" }}>30 days</strong> ·
                  Employee can upload{" "}
                  <strong style={{ color: "#334155" }}>
                    multiple screenshots
                  </strong>{" "}
                  · Each submission appears in{" "}
                  <strong style={{ color: "#334155" }}>Pending</strong> tab ·
                  Link is{" "}
                  <strong style={{ color: "#334155" }}>auto-emailed</strong> on
                  generate
                </p>
              </div>
            </div>
          ...
          {step === 2 && generated && (
            <div style={{ ...S.body, animation: "fadeUp 0.22s ease" }}>
              ...
              >
                ...
                >
                  Email automatically sent to <strong>{email}</strong>. The
                  employee can use the link to submit payment requests with
                  screenshots for <strong>30 days</strong>.
                </p>
              </div>
              ...
              >
                ...
                >
                  This link is <strong>reusable for 30 days</strong>. The
                  employee can open it multiple times and upload a new
                  screenshot each time — every submission creates a separate
                  pending request.
                </p>
              </div>
            </div>
            ...
            {step === 1 && (
              ...
              >
                {generating ? (
                  ...
                  <>
                    <Link2 size={14} /> Generate &amp; send link
                  </>
                )}
              </button>
            )}
