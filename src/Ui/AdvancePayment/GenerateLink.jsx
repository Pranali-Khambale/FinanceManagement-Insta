...
// ONLY CHANGE: buildLink() — removed /#/ prefix entirely.
// Everything else is identical to your original file.
// ─────────────────────────────────────────────────────────────────────────────
import { useState, useEffect } from "react";
import {
  X, Link2, Mail, Copy, RefreshCw, ExternalLink,
  CheckCircle2, AlertCircle, Users, Globe, Loader2, Clock, Send,
} from "lucide-react";
...
// ✅ FIXED: Always BrowserRouter format — no /#/
...
function StepTab({ num, label, active }) {
  return (
    <div style={{
      ...
      borderRadius: 99, background: active ? "#dbeafe" : "#f1f5f9",
      color: active ? "#1d4ed8" : "#94a3b8", fontSize: 12, fontWeight: 600, transition: "all 0.2s",
    }}>
      <span style={{
        width: 16, height: 16, borderRadius: "50%", fontSize: 9, fontWeight: 700,
        display: "flex", alignItems: "center", justifyContent: "center",
        background: active ? "#2563eb" : "#e2e8f0", color: active ? "#fff" : "#94a3b8", flexShrink: 0,
      }}>
        {num}
      </span>
      {label}
...
const S = {
  overlay: {
    ...
    justifyContent: "center", background: "rgba(10,15,30,0.5)",
    backdropFilter: "blur(6px)", WebkitBackdropFilter: "blur(6px)", padding: 16,
  },
  modal: {
  ...
  footer: {
    padding: "12px 20px 16px", borderTop: "1px solid #f1f5f9", background: "#fafbfc",
  ...
  iconClose: {
    ...
    padding: "2px 4px", lineHeight: 1, fontSize: 18,
  },
};
...
const btnPrimary = (disabled) => ({
  ...
  boxShadow: disabled ? "none" : "0 4px 14px rgba(37,99,235,0.3)", whiteSpace: "nowrap",
});
...
export default function GenerateLinkModal({ onClose }) {
  ...
  useEffect(() => {
    advancePaymentService.getPaymentTypes()
      .then(res => {
        const all = (res.data || []).filter(p => ALLOWED_KEYS.includes(p.key));
        setPaymentTypes(all);
  ...
  const handleGenerate = async () => {
    if (!email) { setEmailErr("Employee email is required"); return; }
  ...
  const handleReset = () => {
    setStep(1); setGenerated(null); setCopied(false); setEmailErr(""); setResent(false);
  };
        ...
        <div style={S.modal}>
            ...
            <div style={S.body}>
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            alignItems: "center", gap: 7, padding: "14px 8px 11px",
              ...
              {pt.key && (
                <div style={{
                  ...
                  borderRadius: 9, background: "#f8fafc", border: "1px solid #f1f5f9",
              ...
              <div style={{
                ...
                <p style={{ margin: 0, fontSize: 12, color: "#64748b", lineHeight: 1.65 }}>
                  Valid for <strong style={{ color: "#334155" }}>30 days</strong> ·
                  Link is <strong style={{ color: "#334155" }}>auto-emailed</strong> on generate ·
                  Each submission creates a new pending request
                </p>
              </div>
            </div>
          )}
            ...
            <div style={{ ...S.body, animation: "fadeUp 0.22s ease" }}>
              ...
              <div>
                ...
                <div style={{
                  ...
                  fontSize: 11, color: "#475569", wordBreak: "break-all", lineHeight: 1.75,
                }}>
                  {generated.link}
                </div>
              </div>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                ...
                <a
                  ...
                  style={{
                    display: "flex", alignItems: "center", gap: 5, padding: "7px 13px",
                    borderRadius: 7, background: "#1e293b", color: "#f1f5f9",
                ...
                <button
                  ...
                  style={{
                    ...
                    opacity: resending ? 0.7 : 1, transition: "all 0.15s",
                  }}
                >
                  {resending
              ...
              <div style={{
                display: "flex", alignItems: "flex-start", gap: 8, padding: "10px 12px",
                borderRadius: 9, background: "#fefce8", border: "1px solid #fde68a",
              }}>
                <Clock size={13} color="#b45309" style={{ flexShrink: 0, marginTop: 1 }} />
                <p style={{ margin: 0, fontSize: 12, color: "#92400e", lineHeight: 1.6 }}>
                  This link is <strong>reusable for 30 days</strong>. Each form submission creates a
                  separate pending request. Use <strong>Resend email</strong> above if the employee
          ...
          <div style={S.footer}>
            ...
            {step === 1 && (
              ...
              >
                {generating
                  ? <><Loader2 size={14} style={{ animation: "spin 1s linear infinite" }} /> Generating &amp; emailing…</>
