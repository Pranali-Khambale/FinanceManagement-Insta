...
import {
  X, FileText, CheckCircle2, XCircle, Eye,
  ArrowRight, Calendar, FileImage, ZoomIn, Download,
...
function resolveFileUrl(filePath) {
  ...
  const relativePart =
    uploadsIdx !== -1 ? normalised.slice(uploadsIdx) : normalised.replace(/^\/+/, "");
...
const PAYMENT_TYPES = {
  org_to_emp:    { label: "Org → Employee",        color: "#6366F1", lightBg: "#EEF2FF", borderColor: "#C7D2FE", textColor: "#4338CA" },
  emp_to_emp:    { label: "Employee → Employee",    color: "#0EA5E9", lightBg: "#E0F2FE", borderColor: "#BAE6FD", textColor: "#0369A1" },
  other:         { label: "External / Vendor",      color: "#8B5CF6", lightBg: "#F5F3FF", borderColor: "#DDD6FE", textColor: "#6D28D9" },
  org_to_vendor: { label: "Org → Vendor",           color: "#10B981", lightBg: "#ECFDF5", borderColor: "#A7F3D0", textColor: "#065F46" },
};
...
function InfoTile({ label, value, mono = false }) {
  return (
    <div className="border rounded-xl px-3.5 py-2.5" style={{ background: "rgba(219,234,254,0.35)", borderColor: "rgba(191,219,254,0.6)" }}>
          ...
          <div style={{
            ...
            <button
              ...
              style={{
                ...
                background: "transparent", color: zoom <= 0.5 ? "rgba(255,255,255,0.25)" : "rgba(255,255,255,0.7)",
            ...
            <button
              ...
              style={{
                ...
                cursor: "pointer", fontSize: 11, fontWeight: 700, minWidth: 44,
              }}
              title="Reset zoom (0)"
            >
              {Math.round(zoom * 100)}%
            ...
            <button
              ...
              style={{
                width: 28, height: 28, borderRadius: 6, border: "none",
                background: "transparent", color: zoom >= 3 ? "rgba(255,255,255,0.25)" : "rgba(255,255,255,0.7)",
          ...
          <div style={{ position: "relative", display: "inline-flex" }}>
            ...
            {!error ? (
              ...
              <div style={{
                ...
                gap: 16, padding: 48, borderRadius: 16,
                background: "rgba(255,255,255,0.04)",
                border: "1px solid rgba(255,255,255,0.08)",
                color: "rgba(255,255,255,0.5)",
              }}>
                <FileText size={48} strokeWidth={1} />
                <p style={{ fontSize: 14, margin: 0, fontWeight: 500 }}>Cannot preview this file</p>
                <a
...
function DocCard({ label, name, url: urlProp, filePath, file, pt, badge }) {
  ...
  const resolvedUrl =
    urlProp ||
    (filePath ? resolveFileUrl(filePath) : null) ||
    blobUrl;

  const displayName = name || file?.name || (filePath ? filePath.split(/[\\/]/).pop() : null);
  const isImage = displayName && /\.(png|jpe?g|webp|gif|bmp|svg)$/i.test(displayName);
    ...
    <>
      <div>
        ...
        <div
          ...
          onMouseEnter={e => { if (resolvedUrl) e.currentTarget.style.boxShadow = `0 4px 20px ${pt.color}22`; }}
        ...
        >
          ...
          <div style={{
            ...
            {isImage && resolvedUrl && !imgError ? (
              ...
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={pt.color} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{
              margin: 0, fontSize: 13, fontWeight: 700, color: pt.textColor,
              overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
            }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4 }}>
              {badge && (
                <span style={{
                  ...
                  background: `${pt.color}22`, color: pt.color,
                  textTransform: "uppercase", letterSpacing: ".06em",
                }}>
                  {badge}
                </span>
              )}
              <span style={{ fontSize: 11, color: pt.textColor, opacity: 0.55 }}>
          ...
          {resolvedUrl ? (
            <button
              ...
              style={{
                ...
                background: "#fff",
                color: pt.color, fontSize: 12, fontWeight: 700,
                cursor: "pointer", flexShrink: 0,
                transition: "all 0.15s",
                boxShadow: `0 2px 8px ${pt.color}15`,
              }}
              onMouseEnter={e => { e.currentTarget.style.background = pt.lightBg; e.currentTarget.style.borderColor = pt.color; }}
              onMouseLeave={e => { e.currentTarget.style.background = "#fff"; e.currentTarget.style.borderColor = `${pt.color}55`; }}
            >
              <Maximize2 size={12} /> View
            </button>
          ) : (
            <span style={{ fontSize: 11, color: "#94a3b8", flexShrink: 0 }}>No file</span>
          )}
        </div>
      </div>

      {open && resolvedUrl && (
        <DocViewer src={resolvedUrl} name={displayName} onClose={() => setOpen(false)} />
      )}
    </>
  );
}
...
function FlowDiagram({ req, pt }) {
  ...
  const AmountArrow = () => (
    <div className="flex flex-col items-center gap-1 shrink-0 px-2">
      <div className="flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold" style={{ background: pt.lightBg, color: pt.textColor }}>
        {fmt(req.amount)}
      </div>
  ...
  const VendorBox = () => (
    <div className="flex flex-col items-center gap-1.5 min-w-0">
      ...
      <div className="text-center">
        <p className="text-xs font-bold text-slate-700 truncate max-w-[90px]">{vendorName || "Vendor"}</p>
  ...
  if (paymentType === "org_to_emp")
    return <div className="flex items-center justify-center gap-3 rounded-2xl px-6 py-4 border" style={{ background: pt.lightBg, borderColor: pt.borderColor }}><OrgBox /><AmountArrow /><FromBox /></div>;
  if (paymentType === "org_to_vendor")
    return <div className="flex items-center justify-center gap-3 rounded-2xl px-6 py-4 border" style={{ background: pt.lightBg, borderColor: pt.borderColor }}><OrgBox /><AmountArrow /><VendorBox /></div>;
  if (paymentType === "emp_to_emp") {
    ...
    return <div className="flex items-center justify-center gap-3 rounded-2xl px-6 py-4 border" style={{ background: pt.lightBg, borderColor: pt.borderColor }}><FromBox /><AmountArrow /><ToEmpBox /></div>;
  }
  return <div className="flex items-center justify-center gap-3 rounded-2xl px-6 py-4 border" style={{ background: pt.lightBg, borderColor: pt.borderColor }}><FromBox /><AmountArrow /><VendorBox /></div>;
}
...
export default function ViewDetailModal({ req, onClose, onApprove, onReject }) {
  ...
  // Resolve attachments
  const screenshotAttachment = req.attachments?.find(a => a.role === "screenshot");
  const proofAttachment      = req.attachments?.find(a => a.role === "proof");
  const receiptAttachment    = req.attachments?.find(a => a.role === "receipt");

  const screenshotName     = req.screenshotName || req.screenshot || req.screenshotFile?.name || screenshotAttachment?.name;
  const screenshotUrl      = req.screenshotUrl  || screenshotAttachment?.url  || null;
  const screenshotFilePath = screenshotAttachment?.path || screenshotAttachment?.file_path || req.screenshotFilePath || null;
  const screenshotFile     = req.screenshotFile instanceof File ? req.screenshotFile : null;

  const proofName     = req.proofName  || req.proof  || req.proofFile?.name || proofAttachment?.name;
  const proofUrl      = req.proofUrl   || proofAttachment?.url  || null;
  const proofFilePath = proofAttachment?.path || proofAttachment?.file_path || req.proofFilePath || null;
  const proofFile     = req.proofFile instanceof File ? req.proofFile : null;

  const receiptName     = receiptAttachment?.name;
  const receiptFilePath = receiptAttachment?.path || receiptAttachment?.file_path || null;

  const pt = PAYMENT_TYPES[paymentType] || PAYMENT_TYPES.org_to_emp;

  const hasScreenshot = !!(screenshotName || screenshotFile || screenshotFilePath);
    ...
    <>
      ...
      <div
        style={{
          ...
          boxShadow: "0 32px 80px rgba(15,23,42,0.35), 0 0 0 1px rgba(255,255,255,0.6) inset",
        }}
      >
        {/* Header */}
        <div
          ...
          style={{
            ...
            background: "linear-gradient(135deg, rgba(37,99,235,0.92) 0%, rgba(79,70,229,0.92) 100%)",
            borderBottom: "1px solid rgba(255,255,255,0.08)",
          }}
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-base" style={{ color: "#fff" }}>Request Details</h3>
              <p className="font-mono text-xs mt-0.5" style={{ color: "rgba(255,255,255,0.65)" }}>
          ...
          <button
            onClick={onClose}
            style={{
              width: 32, height: 32, borderRadius: 9, border: "1px solid rgba(255,255,255,0.2)",
        ...
        <div
          ...
          style={{ background: "rgba(248,250,252,0.98)", borderBottom: "1px solid rgba(226,232,240,0.8)" }}
        >
        ...
        {/* Scrollable body */}
        <div className="overflow-y-auto flex-1 p-5 space-y-5" style={{ background: "#fff" }}>
          ...
          {/* Vendor */}
          {(paymentType === "other" || paymentType === "org_to_vendor") && vendorName && (
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2.5">Vendor Details</p>
              <div className="grid grid-cols-2 gap-2">
                <InfoTile label="Vendor Name"   value={vendorName} />
                <InfoTile label="Reference No." value={vendorRef} mono />
                {vendorGst && <InfoTile label="GST No." value={vendorGst} mono />}
              </div>
            </div>
          )}
          ...
          {req.approver_name && (
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2.5">Approver</p>
              <div className="grid grid-cols-2 gap-2">
                <InfoTile label="Name"        value={req.approver_name} />
                <InfoTile label="ID"          value={req.approver_id} mono />
                {req.approver_designation && <InfoTile label="Designation" value={req.approver_designation} />}
              </div>
            </div>
          )}
          ...
          {/* Documents */}
          {hasScreenshot && (
            <DocCard label="Payment Screenshot" name={screenshotName} url={screenshotUrl}
              filePath={screenshotFilePath} file={screenshotFile} pt={pt} badge="Mandatory" />
          )}
          {hasProof && (
            <DocCard label="Supporting Document" name={proofName} url={proofUrl}
              filePath={proofFilePath} file={proofFile} pt={pt} badge="Proof" />
          )}
          {hasReceipt && (
            <DocCard label="Receipt" name={receiptName} filePath={receiptFilePath} pt={pt} badge="Receipt" />
          )}
          ...
          {req.status === "approved" && adjustedIn && (
            <div className="flex items-center gap-3 bg-emerald-50 border border-emerald-100 rounded-xl px-4 py-3">
              ...
              <p className="text-xs text-emerald-800">
                Amount will be deducted in the <span className="font-bold">{adjustedIn}</span> salary cycle.
              </p>
            </div>
          )}
          {req.status === "rejected" && (
