...
// ✅ UPDATED:
//   1. Aadhaar Card split into Front and Back upload cards
...
// Designations that require FARM-ToCli and Medical Certificate
...
const DocumentUpload = ({
  ...
  const fieldRole = isFieldRole(designation);

  // FARM-ToCli: only show + require for field roles in Telecom
  const showFarmToCli = isTelecom && fieldRole;
  // Medical Certificate: mandatory for field roles, optional for others
  const medicalRequired = isTelecom && fieldRole;

  const iconCls =
    "w-8 h-8 text-gray-400 group-hover:text-blue-600 transition-colors";

  const cardProps = { documents, handleFileUpload, handleFileRemove, errors };
    ...
    <div className="space-y-8">
          ...
          <div className="text-sm">
            ...
            {showFarmToCli ? (
              <p className="text-blue-700">
                For <strong>{designation}</strong> (Telecom), the following are
                mandatory: Employee Photo, Aadhaar Card (Front &amp; Back),
                Resume, Bank Passbook, Medical Certificate, and{" "}
                <strong>FARM-ToCli Certificate</strong>. Accepted formats: JPG,
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          {/* FARM-ToCli: only for DT Engineer / Rigger / Technician in Telecom */}
          {showFarmToCli && (
            <DocumentCard
              {...cardProps}
