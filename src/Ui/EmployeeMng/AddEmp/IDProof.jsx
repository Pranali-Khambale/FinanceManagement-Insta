...
/* ─────────────────────────────────────────────
   ...
   Props:
   - formData: the documents object from parent (keyed by fieldName)
   - onFileChange(fieldName, file | null): called on upload or remove
───────────────────────────────────────────── */
const DocumentCard = ({
  title,
  fieldName,
  ...
  // formData is the documents object; each entry is { file, preview, name } or null
  const docEntry = formData?.[fieldName];

  // Resolve the actual File object regardless of storage shape
  const file = docEntry instanceof File ? docEntry : (docEntry?.file ?? null);
  const fileName =
    docEntry instanceof File ? docEntry.name : (docEntry?.name ?? "");
  const fileSize =
    docEntry instanceof File ? docEntry.size : (docEntry?.file?.size ?? 0);
  const preview =
    docEntry instanceof File
      ? docEntry.type?.startsWith("image/")
        ? URL.createObjectURL(docEntry)
        : null
      : (docEntry?.preview ?? null);

  const isUploaded = !!docEntry;
...
/* ─────────────────────────────────────────────
   Main DocumentUploadStep component

   Props:
   - formData: documents state object from AddEmp.jsx
   - onFileChange(fieldName, file | null): unified handler for upload + remove
   - errors: validation errors object
   - department: string
   - requiresFarmToCli: boolean
───────────────────────────────────────────── */
const DocumentUploadStep = ({
  ...
  onFileChange,
  department,
  requiresFarmToCli = false,
}) => {
  const iconCls =
    "w-8 h-8 text-gray-400 group-hover:text-blue-600 transition-colors";

  const rawDept = (department || formData?.department || "")
    .toString()
    .toLowerCase()
    .trim();
    ...
    <div className="space-y-8">
        ...
        <div className="p-4 bg-indigo-50 border border-indigo-200 rounded-lg flex items-start gap-3">
          ...
          <div className="text-sm">
            <p className="font-semibold text-indigo-900 mb-0.5">
              Telecom Department Detected
            </p>
            <p className="text-indigo-700">
              {requiresFarmToCli ? (
                <>
                  Your role (<strong>DT Engineer / Rigger / Technician</strong>)
                  requires a <strong>Medical Certificate</strong> and{" "}
                  <strong>FARM-ToCli Certificate</strong> in addition to the
                  standard documents.
                </>
              ) : (
                <>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {/* ── FIX: fieldName="idPhoto" matches documents.idPhoto in AddEmp state ── */}
          <DocumentCard
          ...
          <DocumentCard
            ...
            description="Upload a single PDF with front & back sides combined"
