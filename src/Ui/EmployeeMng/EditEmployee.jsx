import React, { useState, useRef, useEffect } from "react";
  ...
  const EditEmployee = ({ employee, onClose, onSave, showToast }) => {
    ...
    const [form, setForm] = useState({
      ...
      // ✅ CHANGE 2: uanNumber added to form state — reads from DB (uan_number) or camelCase
      uanNumber:                g(e, "uan_number", "uanNumber"),
    ...
    // ✅ UAN handler — digits only, max 12
    const handleUanChange = (ev) => {
      const v = ev.target.value;
      if (/^\d*$/.test(v) && v.length <= 12) {
        setForm((p) => ({ ...p, uanNumber: v }));
        if (errors.uanNumber) setErrors((p) => ({ ...p, uanNumber: "" }));
      }
    };
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col my-4">
            ...
            <div className="p-6 overflow-y-auto max-h-[74vh]" style={{ scrollbarWidth: "thin" }}>
              <div className="grid grid-cols-2 gap-4">
                ...
                {/* ✅ CHANGE 3: UAN Number field — placed right after "Name on Aadhaar Card" */}
                <div className="col-span-2">
                  <label className="block text-xs font-medium text-gray-600 mb-1 flex items-center gap-1">
                    <Hash className="w-3 h-3 text-gray-500" /> UAN Number
                  </label>
                  <input
                    type="text"
                    name="uanNumber"
                  ...
                  {errors.uanNumber ? (
                    <p className="text-xs text-red-600 flex items-center gap-1 mt-1">
                      <AlertCircle className="w-3 h-3" /> {errors.uanNumber}
                    </p>
                  ) : (
