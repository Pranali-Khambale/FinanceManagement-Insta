...
// ✅ UPDATED: Medical Certificate & FARM-ToCli mandatory ONLY for
//    DT Engineer / Rigger / Technician in Telecom — not for all Telecom users.
...
// ─────────────────────────────────────────────────────────────────────────────
// Medical Certificate + FARM-ToCli are mandatory ONLY for these three roles
// when department is Telecom. All other Telecom roles do NOT require them.
// ─────────────────────────────────────────────────────────────────────────────
...
const RegistrationForm = () => {
  ...
  const mapPrefillToForm = (d) => ({
    ...
    educationalQualification:
      d.educationalQualification || d.educational_qualification || "",
    ...
    familyEmployerContact:
      d.familyEmployerContact || d.family_employer_contact || "",
    emergencyContactName:
      d.emergencyContactName || d.emergency_contact_name || "",
    emergencyContactNo: d.emergencyContactNo || d.emergency_contact_no || "",
    emergencyContactAddress:
      d.emergencyContactAddress || d.emergency_contact_address || "",
    emergencyContactRelation:
      d.emergencyContactRelation || d.emergency_contact_relation || "",
    ...
    localSameAsPermanent:
      d.localSameAsPermanent || d.local_same_as_permanent || false,
  ...
  // ── Restore from sessionStorage on first mount (non-resubmit/rejoin) ──────
  useEffect(() => {
    if (isResubmit) return;
    try {
      const savedForm = sessionStorage.getItem("regForm_formData");
      const savedStep = sessionStorage.getItem("regForm_step");
      if (savedForm) {
  ...
  // ── Auto-save formData + currentStep to sessionStorage on every change ────
  useEffect(() => {
    if (isResubmit) return;
    try {
      sessionStorage.setItem("regForm_formData", JSON.stringify(formData));
      sessionStorage.setItem("regForm_step", String(currentStep));
    } catch {
      // storage quota exceeded — silently ignore
    }
  }, [formData, currentStep, isResubmit]);
  ...
  const handleInputChange = (e) => {
    ...
    // When department changes, clear position so a stale role never carries over
    // and incorrectly triggers FARM-ToCli / Medical requirement
    if (name === "department") {
    ...
    // Aadhaar live-check with debounce
    if (name === "aadhar") {
      setFormData((prev) => ({ ...prev, aadhar: value }));
      if (errors.aadhar) setErrors((prev) => ({ ...prev, aadhar: "" }));
      if (aadharDebounceRef.current) clearTimeout(aadharDebounceRef.current);
      aadharDebounceRef.current = setTimeout(() => checkAadhar(value), 600);
      return;
    }
  ...
  const clearSavedDraft = () => {
    sessionStorage.removeItem("regForm_formData");
    sessionStorage.removeItem("regForm_step");
    setFormData(emptyForm);
    setCurrentStep(1);
    setErrors({});
  };

  // ── Derive requiresRoleDocs (reused in validateStep + render) ─────────────
  // Medical Certificate & FARM-ToCli are ONLY required for DT Engineer,
  // Rigger, and Technician in Telecom. All other Telecom roles do NOT require them.
  const requiresRoleDocs =
    formData.department?.toLowerCase() === "telecom" &&
    FARM_TO_CLI_POSITIONS.includes(
      (formData.position || "").toLowerCase().trim(),
    );
  ...
  const validateStep = (step) => {
    ...
    if (step === 4) {
      // Always required for every employee regardless of department
      ...
      // Medical Certificate & FARM-ToCli are ONLY required for
      // DT Engineer, Rigger, and Technician in Telecom.
      // They are optional for all other roles — including other Telecom roles.
      if (requiresRoleDocs) {
  ...
  // Check if a saved draft exists (for the restore banner)
  const hasSavedDraft = (() => {
    try {
      return !!sessionStorage.getItem("regForm_formData");
    } catch {
      return false;
    }
  })();
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-10 max-w-md w-full text-center">
          ...
          <div
            className={`rounded-xl p-4 text-left ${isRejoin ? "bg-indigo-50" : "bg-blue-50"}`}
          >
            <p
              className={`text-sm font-semibold mb-2 ${isRejoin ? "text-indigo-800" : "text-blue-800"}`}
            >
              What happens next?
            </p>
            <ul
              className={`text-sm space-y-1 ${isRejoin ? "text-indigo-700" : "text-blue-700"}`}
            >
              {isRejoin ? (
                <>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 py-8 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        {hasSavedDraft && !isRejoin && !isResubmit && (
          <div className="mb-5 flex items-center justify-between gap-3 bg-amber-50 border border-amber-300 rounded-xl px-4 py-3 shadow-sm">
            <div className="flex items-center gap-2 text-sm text-amber-800">
              <Save className="w-4 h-4 text-amber-500 flex-shrink-0" />
              <span>
                <strong>Draft restored.</strong> Your previously saved progress
                has been loaded automatically.
              </span>
            </div>
            <button
              onClick={clearSavedDraft}
              className="text-xs font-semibold text-red-500 hover:text-red-700 whitespace-nowrap underline transition-colors"
            >
              Clear &amp; start fresh
            </button>
          </div>
        ...
        {currentStep === 1 && showRejoinOption && (
          <div className="mb-6">
            ...
            >
              <div
                className={`h-1.5 w-full ${isRejoin ? "bg-gradient-to-r from-indigo-500 to-violet-500" : "bg-amber-400"}`}
              />
              {!isRejoin && (
                <div className="p-5">
                  <div className="flex items-start gap-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
          ...
          {currentStep === 4 && (
            <Documents
              ...
              department={formData.department}
              // ✅ Medical + FARM-ToCli mandatory ONLY for DT Engineer / Rigger / Technician
              // All other Telecom roles (Coordinator, Manager, etc.) do NOT require them
              requiresFarmToCli={requiresRoleDocs}
            />
          )}
          ...
          <div className="mt-8 pt-6 border-t border-gray-200 space-y-3">
            ...
            {saveFlash && (
              <div className="flex items-center justify-center gap-2 py-2.5 px-4 bg-green-50 border border-green-300 rounded-xl text-sm text-green-800 font-semibold">
                <CheckCircle className="w-4 h-4 text-green-600 flex-shrink-0" />
                Progress saved! You can safely close or refresh this page.
              </div>
            )}
            ...
            <button
              type="button"
              onClick={handleSave}
              className="w-full flex items-center justify-center gap-2 px-5 py-2.5 bg-white border-2 border-dashed border-gray-300 hover:border-green-400 hover:bg-green-50 text-gray-400 hover:text-green-700 rounded-xl font-semibold text-sm transition-all duration-200 group"
            >
              <Save className="w-4 h-4 group-hover:scale-110 transition-transform" />
              Save Progress
            </button>

            {/* Documents note on step 4 */}
            {currentStep === 4 && (
              <p className="text-xs text-center text-amber-600 bg-amber-50 border border-amber-200 rounded-lg py-2 px-3">
                <strong>Note:</strong> Uploaded files are not saved in the draft
                — please re-upload documents after a page refresh.
              </p>
            )}

            {/* Back / Step indicator / Next–Submit */}
            <div className="flex items-center justify-between">
              <button
                type="button"
                onClick={handleBack}
