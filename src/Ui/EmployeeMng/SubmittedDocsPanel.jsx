...
import {
  FileText, Shield, Camera, CheckCircle, Clock, Eye,
  Download, ExternalLink, Loader, AlertCircle, RefreshCw,
...
// ── Doc type metadata ─────────────────────────────────────────────────────────
const DOC_META = {
  signed_kye:       { label: 'Signed KYE Form',      icon: <FileText className="w-5 h-5 text-blue-600" />,   color: 'blue',   required: true },
  bgv_form:         { label: 'BGV Form',              icon: <Shield   className="w-5 h-5 text-indigo-600" />, color: 'indigo', required: false },
  email_screenshot: { label: 'Approval Email Screenshot', icon: <Camera className="w-5 h-5 text-green-600" />,  color: 'green',  required: false },
  other:            { label: 'Other Document',        icon: <File     className="w-5 h-5 text-gray-500" />,   color: 'gray',   required: false },
};
      ...
      <div className="flex items-center justify-between px-6 py-3 flex-shrink-0"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-blue-200 text-xs">{idx + 1} of {docs.length}</p>
          </div>
        </div>
...
export const SubmittedDocsPanel = ({ empDbId, employeeName, showToast }) => {
  ...
  const handleMarkReviewed = async (docId) => {
    ...
    try {
      const res  = await fetch(`${BASE_URL}/employee-docs/mark-reviewed/${docId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
      const data = await res.json();
      if (data.success) {
        setDocs(prev => prev.map(d => d.id === docId ? { ...d, reviewed: true } : d));
  ...
  if (error) {
    return (
      <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-xl px-4 py-3">
        ...
        <button onClick={fetchDocs} className="ml-auto text-xs text-red-600 underline">Retry</button>
      </div>
    );
  }
    ...
    <>
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between px-4 py-2.5 bg-blue-50 border border-blue-100 rounded-xl">
          ...
          <button onClick={fetchDocs} className="text-blue-500 hover:text-blue-700">
            ...
            <div key={doc.id}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  {!doc.reviewed && (
                    <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${colors.bg} ${colors.text} border ${colors.border}`}>
                      NEW
                    </span>
                  )}
                </div>
...
// Small inline badge for the employee card in PendingApprovals
export const SubmittedDocsBadge = ({ empDbId, docsSubmitted }) => {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold border ${
      ...
      {unreviewed > 0
        ? <><FileText className="w-2.5 h-2.5" /> {unreviewed} doc{unreviewed !== 1 ? 's' : ''} pending</>
