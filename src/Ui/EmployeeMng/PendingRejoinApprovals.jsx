...
const Lightbox = ({ docs, startIndex = 0, onClose }) => {
  ...
  useEffect(() => {
    setImgError(false);
    setResolvedUrl(null);
    getDocUrl(doc?.path).then(setResolvedUrl);
  }, [idx, doc?.path]);
  ...
  if (
    ...
    return (
      ...
      >
        <Loader className="w-8 h-8 text-indigo-300 animate-spin" />
      </div>
    );
  }
  if (!url) return null;
...
const DocLightbox = ({ docs, startIndex = 0, onClose }) => {
  ...
  const meta =
    SUBMITTED_DOC_META[doc?.document_type] || SUBMITTED_DOC_META.other;
      ...
      <div className="mx-5 mb-4 rounded-xl border border-dashed border-indigo-200 bg-indigo-50/40 px-4 py-3 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] text-indigo-400 mt-0.5">
            Employee will upload signed KYE, BGV form &amp; email screenshot
...
const RejoinDocReviewCard = ({ emp, onAllReviewed, showToast }) => {
  ...
  const fetchDocs = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${BASE_URL}/employee-docs/submissions/${emp.id}`,
      );
      const data = await res.json();
      if (data.success) setDocs(data.data || []);
        ...
        <div className="px-5 pt-4 pb-3 border-b border-gray-100">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-3.5">
              <div className="relative">
                <div
                  className="w-12 h-12 rounded-full flex items-center justify-center text-indigo-700 font-bold text-base flex-shrink-0"
                  style={{
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {!allDone && pending > 0 && (
                    ...
                    >
                      <Bell className="w-2.5 h-2.5" /> {pending} Doc
                      {pending > 1 ? "s" : ""} Pending
                    </span>
                  )}
                  {allDone && (
        ...
        <div className="px-5 py-3 border-b border-gray-50">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            {[
              ...
              {
                icon: <Phone className="w-3.5 h-3.5 text-indigo-500" />,
                label: "Phone",
                value: emp.phone,
              },
              {
                ...
                value: fmtDateShort(emp.docs_submitted_at),
              },
            ].map((item, i) => (
              <div
                key={i}
              ...
              >
                ...
                <div className="min-w-0">
                  <p className="text-[9px] font-medium text-indigo-400 uppercase tracking-wide leading-none mb-0.5">
                    {item.label}
                  </p>
                  ...
                  <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-green-50 border border-green-200 mt-1">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-green-600 mt-0.5">
                        Rejoin onboarding is complete for this employee
                      </p>
                    </div>
                  </div>
                )}
...
const RejoinDocsPendingReviewSection = ({ showToast }) => {
  ...
  const totalUnreviewed = employees.reduce(
    (sum, e) => sum + (parseInt(e.unreviewed_count) || 0),
    0,
  );
            ...
            {totalUnreviewed > 0 && (
              <div className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 rounded-full flex items-center justify-center border-2 border-white">
                <span className="text-[9px] font-bold text-white">
                  {totalUnreviewed > 9 ? "9+" : totalUnreviewed}
                </span>
              </div>
            )}
          </div>
...
const FullDetailModal = ({ sub, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (!pending.length) return;
    Promise.all(
      pending.map((d) => getDocUrl(d.path).then((url) => [d.type, url])),
    ).then((pairs) => {
        ...
        >
          ...
          >
            <div className="flex items-center gap-4">
              <div
                className="w-14 h-14 rounded-xl flex items-center justify-center font-bold text-white text-lg flex-shrink-0 relative"
                style={{
                  background: "linear-gradient(135deg,#4f46e5,#7c3aed)",
                }}
              >
                {(sub.first_name?.[0] || "N").toUpperCase()}
                {(sub.last_name?.[0] || "A").toUpperCase()}
              ...
              <div>
                ...
                <p className="text-indigo-200 text-sm">
                  {sub.position || sub.designation || "Position not specified"}{" "}
                  &bull; {sub.department || "Department not specified"}
                </p>
                {sub.employee_id && (
                  <p className="text-indigo-300 text-xs mt-0.5">
                    Previous ID: {sub.employee_id}
                  </p>
                )}
              </div>
            </div>
          ...
          <div
            className="px-6 py-3 flex items-center gap-3 border-b border-indigo-100"
          ...
          >
            ...
            <p className="text-xs text-indigo-700">
              <span className="font-bold">Rejoin Request</span> — Previously
              inactive employee has submitted updated information. Approving
              will assign a <strong>new Employee ID</strong> and{" "}
              <strong>replace old data with submitted data</strong>. Declining
              will <strong>restore original data</strong> and return employee to
              Inactive.
            </p>
          </div>
              ...
              <div className="space-y-5">
                <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <Field
                      ...
                      value={sub.father_husband_name}
                    />
                    <Field
                      label="Date of Birth"
                      value={fmt(sub.date_of_birth)}
                    />
                    ...
                    <Field
                      ...
                      value={sub.educational_qualification}
                    />
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Field
                      label="Contact Name"
                      value={sub.emergency_contact_name}
                      span={2}
                    />
                    <Field
                      label="Contact No"
                      value={sub.emergency_contact_no}
                    />
                    <Field
                      label="Relation"
                      value={sub.emergency_contact_relation}
                    />
                    <Field
                      label="Address"
                      value={sub.emergency_contact_address}
                      span={2}
                    />
                  </div>
                </div>
                <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Field
                      ...
                      value={sub.permanent_address}
                      span={2}
                    />
                    <Field
                      label="Local Address"
                      value={
                        sub.local_same_as_permanent
                          ? "Same as permanent"
                          : sub.local_address
                      }
                      span={2}
              ...
              <div className="space-y-5">
                <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <Field
                      label="Designation"
                      value={sub.position || sub.designation}
                    />
                    <Field
                      label="Employment Type"
                      value={sub.employment_type}
                    />
                    <Field label="Joining Date" value={fmt(sub.joining_date)} />
                    <Field
                      label="Reporting Manager"
                      value={sub.reporting_manager}
                    />
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
                  ...
                  {isTelecom && (
                    <div className="flex items-center gap-2 mb-3 p-2.5 bg-indigo-50 border border-indigo-200 rounded-lg">
                      ...
                      <p className="text-xs text-indigo-700 font-medium">
                        FARM-ToCli Certificate is required for this Telecom
                        employee.
                      </p>
                    </div>
                  )}
                  ...
                  {availableDocs.length > 0 && (
                    <button
                      onClick={() =>
                        setLightbox({ docs: availableDocs, startIndex: 0 })
                      }
    ...
    <div className="fixed inset-0 z-[9999] backdrop-blur-sm bg-black/40 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="px-6 py-5">
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-4">
            <p className="text-xs text-amber-700">
              <strong>Note:</strong> Declining will restore this employee's data
              to its pre-submission state and return their status to{" "}
              <strong>Inactive</strong>. HR can send a new rejoin invite later.
            </p>
          </div>
        ...
        <div className="px-6 pb-5 flex gap-3 justify-end">
          ...
          <button
            onClick={() =>
              onConfirm(reason.trim() || "Rejoin request declined by HR.")
            }
            disabled={isLoading}
...
const SubmissionCard = ({
  ...
  const name = `${sub.first_name || ""} ${sub.last_name || ""}`.trim();
  const submittedAt =
    sub.rejoin_requested_at || sub.updated_at
      ? new Date(sub.rejoin_requested_at || sub.updated_at).toLocaleString(
          "en-IN",
          {
            day: "2-digit",
            month: "short",
  ...
  return (
    <>
      {showDetails && (
        <FullDetailModal sub={sub} onClose={() => setShowDetails(false)} />
      )}
      ...
      >
            ...
            <div className="flex items-center gap-2 mt-1.5 flex-wrap">
              ...
              {isTelecom && (
                <span className="inline-flex items-center gap-1 text-[10px] font-bold text-violet-700 bg-violet-100 px-2 py-0.5 rounded-full border border-violet-200">
                  <Radio className="w-2.5 h-2.5" /> Telecom — FARM-ToCli
                  Required
                </span>
              )}
              {docsSubmitted && (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 px-5 py-3 bg-white border-t border-indigo-50">
          {[
            ...
            {
              icon: <Banknote className="w-3.5 h-3.5 text-indigo-400" />,
              label: "Designation",
              value: sub.position || sub.designation,
            },
            {
                  ...
                  {sub.documents.map((doc, i) => {
                    ...
                    return (
                      <DocLinkButton
                        key={i}
                        rawPath={rawPath}
                        label={doc.type || doc.document_type}
                      />
                    );
                  })}
                </div>
...
const PendingRejoinApprovals = () => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const res = await fetch(
        `${BASE_URL}/employees/${sub.id}/pending-rejoin`,
        {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
        },
      ...
      if (data.success) {
        showToast(
          `🗑️ Rejoin request deleted — ${sub.first_name} ${sub.last_name} restored to Inactive`,
          "success",
        );
        setSubmissions((prev) => prev.filter((s) => s.id !== sub.id));
      } else {
  ...
  return (
    <div className="p-8 min-h-screen bg-gray-50">
      ...
      {toast && (
        <div
          className={`fixed top-20 right-4 z-[9999] flex items-center gap-3 px-4 py-3 rounded-xl border shadow-lg text-sm font-medium min-w-[260px] max-w-[400px] ${
            toast.type === "success"
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold text-gray-900">
                Pending Rejoin Approvals
              </h1>
              {submissions.length > 0 && (
                <span
                  className="inline-flex items-center justify-center min-w-[28px] h-7 px-2 rounded-full text-white text-xs font-bold"
                  style={{
                    background: "linear-gradient(135deg,#4f46e5,#7c3aed)",
                  }}
                >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
          {[
            ...
            },
            {
              ...
              value: fmtDateTime(
                submissions[0]?.rejoin_requested_at ||
                  submissions[0]?.updated_at,
              ),
              color: "#d97706",
            },
            {
      ...
      <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4 mb-6 flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-indigo-800">
            Before approving a rejoin request
          </p>
          <p className="text-xs text-indigo-600 mt-0.5">
            <strong>Approve</strong> → new submitted data is saved, a new
            Employee ID is assigned, approval email with KYE PDF &amp; upload
            link sent to employee. &nbsp;|&nbsp;
            <strong>Decline</strong> → original pre-submission data is restored,
            employee returns to Inactive. &nbsp;|&nbsp;
            <strong>Delete</strong> → same as Decline but without sending an
            email to the employee. &nbsp;|&nbsp;
            <strong className="text-violet-700">After approval</strong> →
            employee uploads signed KYE, BGV form &amp; email screenshot; HR
            reviews docs inline below each card. &nbsp;|&nbsp;
            <strong className="text-violet-700">Telecom employees</strong> →
            FARM-ToCli Certificate is mandatory; verify it is uploaded before
            approving.
          </p>
        </div>
      </div>
          ...
          {submissions.length === 0 ? (
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-16 text-center">
              ...
              <p className="text-gray-500 text-sm mb-6">
                All caught up! No employees have submitted rejoin requests yet.
              </p>
              <button
                onClick={() => navigate("/employee/management")}
