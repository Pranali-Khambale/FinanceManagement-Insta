...
//  4. Employment Type filter (IT / Telecom / All Types)
//  5. Circle / Location filter uses hardcoded list (synced with
//     EmploymentDetailsStep) instead of dynamic population from employee data
//  6. Search also matches against circle and designation
//  7. PROFESSIONAL FILTER BAR — no emojis, clean chevron dropdowns,
//     per-filter accent colors, active tag pills, clear-all badge
// ─────────────────────────────────────────────────────────────────────────────
...
// ─── Chevron icon used inside selects ─────────────────────────────────────
const ChevronDown = () => (
  <svg
...
const PayslipViewModal = ({ employee, onClose }) => {
  ...
  const DeductRow = ({ label, amount, isPositive, subdued }) => (
    <tr className="text-[12px]">
      <td className={`py-1 ${subdued ? "text-slate-400 italic" : "text-slate-600"}`}>
        {label}
      </td>
      <td
...
const PayrollTable = ({
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100">
      ...
      {/* TOOLBAR                                                             */}
      {/* ═══════════════════════════════════════════════════════════════════ */}
      <div className="px-6 pt-5 pb-0 flex flex-col gap-0">
        ...
        <div className="flex items-center justify-between gap-3 pb-4 border-b border-slate-100">
          ...
          <div className="flex items-center bg-slate-100 rounded-[9px] p-[3px] gap-[2px]">
            {[
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-[6px] rounded-[7px] text-[13px] transition-all ${
                  activeTab === tab.key
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-[6px] px-4 py-[7px] rounded-lg text-[13px] font-medium bg-indigo-700 hover:bg-indigo-800 text-white transition-colors whitespace-nowrap"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-[6px] px-4 py-[7px] rounded-lg text-[13px] font-medium bg-emerald-700 hover:bg-emerald-800 text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
            >
              {exporting ? (
                <>
                  <svg className="w-[14px] h-[14px] animate-spin flex-shrink-0" fill="none" viewBox="0 0 24 24">
        ...
        <div className="flex items-center gap-2 flex-wrap py-3 border-b border-slate-100">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-[30px] pr-7 py-[7px] text-[13px] border border-slate-200 rounded-lg bg-slate-50 focus:outline-none focus:ring-[3px] focus:ring-blue-100 focus:border-blue-300 focus:bg-white transition-all w-[220px]"
            />
            {search && (
              <button
                onClick={() => setSearch("")}
          ...
          {/* Vertical divider */}
          <div className="w-px h-[22px] bg-slate-200 mx-[2px] flex-shrink-0" />
          ...
          <span className="ml-auto text-[12px] text-slate-400 whitespace-nowrap">
            {filtered.length} of{" "}
            {employees.filter((e) => tabMap[activeTab]?.includes(e.status)).length} employees
          </span>
        </div>

        {/* ── Row 3: Active filter tag pills ────────────────────────────── */}
        {activeFiltersCount > 0 && (
