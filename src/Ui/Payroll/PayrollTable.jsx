...
function computePayslip(emp) {
  ...
  const totalDeduction =
    empPfDed + employerPf + ptDed + tds + otherDed + advDed;
...
const PayslipViewModal = ({ employee, onClose }) => {
  ...
  const DeductRow = ({ label, amount, isPositive, subdued }) => (
    <tr className="text-[12px]">
      ...
      <td
        className={`py-1 text-right font-medium ${isPositive ? "text-emerald-600" : subdued ? "text-slate-400 italic" : "text-red-500"}`}
      >
        {isPositive
          ? `+ ${fmtINR(amount)}`
          : n(amount) > 0
      ...
      >
        ...
        <div className="overflow-y-auto flex-1">
            ...
            <div className="px-5 py-4">
              ...
              <table className="w-full border-collapse">
                ...
                <tbody>
                  ...
                  <EarningRow
                    label="Medical allow."
                    gross={n(employee.medicalAllowance)}
                    earned={n(employee.medicalAllowance) * ratio}
                  />
          ...
          <p className="px-6 pt-2 text-[11px] text-slate-400">
            ℹ️ PF: 12% Basic (employee) + 12% Basic (employer) = 24% total —
            both deducted from employee salary &nbsp;|&nbsp; PT = ₹200/month ·
            ₹300 in February &nbsp;|&nbsp; PT for Female: applicable only if
            Gross &gt; ₹25,000 &nbsp;|&nbsp; Any field can be set to ₹0 to
            exempt the employee
          </p>
...
const PayrollTable = ({
  ...
  const handlePay = async (emp) => {
    if (!emp.payrollRecordId) {
      showToast(
        "⚠️ No payroll record found. Please save attendance/salary first.",
        "error",
      );
      return;
    }
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100">
      ...
      <div className="px-6 py-3 border-t border-slate-100 flex items-center justify-between">
        <p className="text-xs text-slate-400">
          ...
          &nbsp;·&nbsp;
          {forMonth} · {correctMonthDays} days &nbsp;·&nbsp; PF = 12% Emp + 12%
          Co. = 24% total &nbsp;·&nbsp; PT = ₹
          {/february/i.test(forMonth || "") ? "300 (Feb)" : "200"}/month
          &nbsp;·&nbsp; ♀ PT only if Gross &gt; ₹25,000 &nbsp;·&nbsp; Set any
          field to ₹0 to exempt
        </p>
      </div>
    </div>
  );
